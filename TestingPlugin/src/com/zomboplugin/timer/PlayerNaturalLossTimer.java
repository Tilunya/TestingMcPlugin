...
public class PlayerNaturalLossTimer extends BukkitRunnable {
	...
	public void run() {
		synchronized (PersistData.get_playerConnectedList()) {
			for (int i = 0; i < PersistData.get_playerConnectedList().size(); i++) {
				...
				playerData.get_state().lostNaturalExhaution(playerData);
				playerData.get_state().lostNaturalHydration(playerData);
			}
		}
	}
